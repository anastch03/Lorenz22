...
std::string Mapper::noPunc(const char *input)
{ // gets rid of input punctuation
...
std::string Mapper::fmap2(const char *input)
{ // gets rid of spaces and replace with '-'
    std::string str = "";
    for (const char *i = input; *i != '\0'; i++)
    {
        if (*i == ' ')
...
int Mapper::asciiToBit(char c)
{ // convert a char's ascii value to 6-bit value
...
int Mapper::bitToAscii(int bit)
{ // convert ascii value to 6-bit value
    return Mapper::get_mapping()->at(bit);
}

void Mapper::rotate()
{ // rotate mapping right by 1
